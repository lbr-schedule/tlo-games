...
app.post('/api/roulette/bet', async (req, res) => {
    ...
    if (LOCAL_TEST_MODE) {
        ...
        try {
            ...
            console.log('🔴 rowsAffected=', deductResult.rowsAffected, 'username:', username, 'amount:', amount);
            if (deductResult.rowsAffected === 0) {
                return res.json({ success: false, message: '餘額不足，無法下注' });
            }
            // 再次確認扣款成功
            const verify = await rouletteDb.execute({ sql: 'SELECT score FROM players WHERE username = ?', args: [username] });
            if (!verify.rows || verify.rows.length === 0) {
                console.log('⚠️ 扣款後查無玩家:', username);
                return res.json({ success: false, message: '資料庫錯誤' });
            }
            console.log('🔴 扣款後餘額確認:', verify.rows[0].score);
        } catch(e) {
            console.log('🔴 扣款失敗:', e.message);
            return res.json({ success: false, message: '下注失敗，請稍後再試' });
        }
    }
